...
// The navigable window. Forward, one year past the current one so you can plan
// next year. Back, a fixed floor rather than a sliding span — there is no data
// before this, and a floor means older years stay one click away instead of
// dropping off the picker as time passes. The arrows and the picker both derive
// from these, so they can never offer different ranges.
const EARLIEST_FY = "2016-17";

export const maxFinancialYear = (anchor = currentFinancialYear()) => shiftFinancialYear(anchor, 1);
export const minFinancialYear = () => EARLIEST_FY;

// Newest first, for the picker.
export const financialYearOptions = (anchor = currentFinancialYear()) => {
  const out = [];
  for (let y = maxFinancialYear(anchor); y >= EARLIEST_FY; y = shiftFinancialYear(y, -1)) out.push(y);
  // A clock set before the floor would otherwise leave nothing to choose from.
  return out.length ? out : [anchor];
};

// Well-formed *and* inside the window the nav offers, so a year typed into the
// URL cannot reach somewhere the arrows and picker refuse to go.
export const isFinancialYearInRange = (fy, anchor = currentFinancialYear()) =>
  isFinancialYear(fy) && fy >= minFinancialYear() && fy <= maxFinancialYear(anchor);
...
// "2026-27" -> "FY 26–27", for tight spots like the profile card.
export const fyLabelShort = (fy) => `FY ${String(fy).slice(2).replace("-", "–")}`;
...
// Weeks are contained within their month. Week 1 runs from the 1st to the first
// Sunday — often a part week — then weeks 2 onward are full Monday-to-Sunday,
// and the last row takes whatever is left so no day of the month falls outside
// the five rows. July 2026: 1–5, 6–12, 13–19, 20–26, 27–31.
//
// The trade-off is that a real pay week spanning month end is split across two
// columns, which is what keeps each month's income attributable to that month.
//
// Built from year/month/day integers throughout, never parsed from a string, so
// these are calendar dates rather than instants and no timezone can shift them.
export const weekRange = (fy, monthIndex, weekIndex) => {
  const { year, month } = monthDate(fy, monthIndex);
  const lastDay = daysInLedgerMonth(fy, monthIndex);
  const firstDow = new Date(year, month, 1).getDay();
  const firstSunday = firstDow === 0 ? 1 : 8 - firstDow;

  const startDay = weekIndex === 0 ? 1 : firstSunday + 1 + (weekIndex - 1) * 7;
  if (startDay > lastDay) return null;

  const isLastRow = weekIndex === WEEKS.length - 1;
  const endDay = Math.min(isLastRow ? lastDay : weekIndex === 0 ? firstSunday : startDay + 6, lastDay);

  return { start: new Date(year, month, startDay), end: new Date(year, month, endDay) };
};

export const weekRangeLabel = (fy, monthIndex, weekIndex) => {
  const range = weekRange(fy, monthIndex, weekIndex);
  const { year, month } = monthDate(fy, monthIndex);
  if (!range) {
    return `${CAL_MONTHS_SHORT[month]} ${year} has no week ${weekIndex + 1}`;
  }
  const day = (d) => `${d.getDate()} ${CAL_MONTHS_SHORT[d.getMonth()]}`;
...
// Its heading is a constant, not stored text — like "Fixed cost" and
// "Variable cost", it names a fixed part of the sheet. Applied on load, so it
// reads the same across every financial year, past and future alike.
...
// Folds the pinned source down to the single row it renders as.
//
// Already one row (or none): pass it through. Migrating from the five week
// rows: concatenate their lines, and turn any week that held a plain typed
// amount into a line of its own, so a figure recorded before this change is
// carried across rather than dropped.
const collapseToOneRow = (rawRows) => {
  const weeks = rawRows.map((r) => cleanRow(r));
  if (weeks.length <= 1) {
    return { ...(weeks[0] ?? cleanRow(null)), label: OTHER_INCOME_LABEL };
  }

  const entries = months.map((_, i) => {
    const lines = [];
    for (const w of weeks) {
      if (w.entries[i].length) lines.push(...w.entries[i]);
      else if (w.values[i]) lines.push({ day: 0, note: w.label, amount: w.values[i] });
    }
    return sortEntries(lines);
  });

  return {
    label: OTHER_INCOME_LABEL,
    values: entries.map(entriesTotal),
    entries,
  };
};
...
export const normaliseLedger = (raw, fy) => {
  ...
  let cleaned = sources.map((src) => ({
    ...
    // The pinned source is re-shaped to a single row further down.
    rows: WEEKS.map((w, i) => cleanRow(src?.rows?.[i], w)),
    rawRows: Array.isArray(src?.rows) ? src.rows : [],
  }));
  ...
  // The heading is the constant, not whatever a given year happens to store, so
  // it reads the same in every financial year.
  other.label = OTHER_INCOME_LABEL;
  // Irregular income has no use for weeks — its lines carry their own day — so
  // it is one row per month rather than five.
  other.rows = [collapseToOneRow(other.rawRows)];
  cleaned.push(other);

  out.incomeSources = cleaned.map(({ rawRows, ...src }) => src);
...
// The financial year a real calendar date falls in, plus that month's column
// index within the FY grid (0 = Jul … 11 = Jun). The dashboard's Income /
// Expense card uses this to walk backwards over real months, which may cross a
// 1 July boundary and so span two ledgers.
export const ledgerMonthOf = (date) => {
  const m = date.getMonth(); // 0 = Jan
  const startYear = m >= 6 ? date.getFullYear() : date.getFullYear() - 1;
  return { fy: makeFy(startYear), monthIndex: (m + 6) % 12 };
};

// Income and expense totals per FY-month for a raw ledger document (or null,
// which yields that year's seed figures). Returns { income:[12], expense:[12] }
// in the same Jul-first order as `months`.
export const ledgerMonthlyTotals = (rawData, fy) => {
  const { fixedCost, variableCost, incomeSources } = normaliseLedger(rawData, fy);
  return {
    income: columnTotals(incomeSources.flatMap((s) => s.rows)),
    expense: columnTotals([...fixedCost, ...variableCost]),
  };
};

// A rough full-year income figure for a raw ledger document, built from typical
// weekly pay so a month with only a week or two entered doesn't skew it.
//
// Every non-zero week counts, including the part-weeks at the start and end of a
// month — but each is weighted by how many days it actually spans, so a 4-day
// week of $1,000 reads as the same rate as a 7-day week of $1,750 rather than
// dragging the average down. Total recorded income over total week-fraction is
// the weekly rate; that is annualised across 52 weeks. The irregular "other
// income" bucket is left out; it's lumpy by nature. Returns 0 when no regular
// income has been recorded for the year.
export const projectedYearIncome = (rawData, fy) => {
  const regular = normaliseLedger(rawData, fy).incomeSources.filter((s) => !isOtherIncome(s));
  let income = 0;
  let weeksCovered = 0;
  for (let mi = 0; mi < months.length; mi++) {
    for (let wi = 0; wi < WEEKS.length; wi++) {
      const total = regular.reduce((sum, s) => sum + (s.rows[wi]?.values[mi] || 0), 0);
      if (total <= 0) continue;
      const range = weekRange(fy, mi, wi);
      if (!range) continue;
      const days = Math.round((range.end - range.start) / 86400000) + 1;
      income += total;
      weeksCovered += Math.min(days, 7) / 7;
    }
  }
  if (weeksCovered <= 0) return 0;
  return (income / weeksCovered) * 52;
};
