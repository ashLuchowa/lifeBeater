...
const years = {
  "2025-26": { fixedCost, variableCost, mainIncome, sideIncome, otherIncome },
};
...
// The five editable groups, in render order. The Expenses / Income totals and
// the summary cards are built on this shape, so groups are fixed — rows inside
// them are not.
export const GROUPS = [
  { key: "fixedCost", label: "Fixed cost", family: "warm" },
  { key: "variableCost", label: "Variable cost", family: "warm" },
  { key: "mainIncome", label: "Main income", family: "cool" },
  { key: "sideIncome", label: "Side income (gross)", family: "cool" },
  { key: "otherIncome", label: "Other income (net)", family: "cool" },
];

export const blankRow = (label = "") => ({ label, values: months.map(() => 0) });

// Coerce anything loaded from the database into the shape the page renders,
// so a hand-edited or partial row can never crash the grid.
export const normaliseLedger = (raw, fy) => {
  const seed = ledgerFor(fy);
  const out = {};
  for (const g of GROUPS) {
    const rows = Array.isArray(raw?.[g.key]) ? raw[g.key] : seed[g.key];
    out[g.key] = rows.map((r) => ({
