...
// "24 – 30 Aug, 2026", or "31 Aug – 6 Sep, 2026" across a month boundary. Kept
// short: this sits in the top bar, which has little room on a phone.
export function formatWeek(s) {
  const a = fromStr(weekStart(s));
  const b = fromStr(weekEnd(s));
  const mon = (d) => MONTHS[d.getMonth()].slice(0, 3);
  const left = a.getMonth() === b.getMonth() ? `${a.getDate()}` : `${a.getDate()} ${mon(a)}`;
